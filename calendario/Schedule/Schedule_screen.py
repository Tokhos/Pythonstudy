...
class ScheduleScreen(tk.Toplevel):
    def __init__(self, master):
        ...
        self.schelude_treeview = ttk.Treeview(self, columns=('ID', 'User', 'Professional', 'Date', 'Status'), show='headings')
        self.schelude_treeview.heading('ID', text='ID')
        ...
        self.schelude_treeview.bind("<Double-1>", self.on_schedule_double_click)
    ...
    def show_info_on_treeview(self):
        ...
        scheludes_data = self.cursor.execute('''
            SELECT schedule.id_schedule, users.name, professional.name_pro, schedule.available_data, users.status
            FROM schedule
        ...
        for schelude in scheludes_data:
            id_schedule, user_name, professional_name, available_data, status = schelude
            status_label = "Active" if status == 1 else "Inactive"
            self.schelude_treeview.insert('', 'end', values=(id_schedule, user_name, professional_name, available_data, status_label))
    #status update        
    ...
    def delete_schedule(self):
        ...
        selected_values = self.schelude_treeview.item(selected_item, 'values')
        user_name = selected_values[0]  
        professional_name = selected_values[1]  
        available_data = selected_values[2]  
    ...
    def on_schedule_double_click(self, event):
        ...
        selected_items_schedule = self.schelude_treeview.selection()
        
        if len(selected_items_schedule) > 0:
            
            selected_item_schedule = selected_items_schedule[0]
            item_values = self.schelude_treeview.item(selected_item_schedule, "values")
           
            if item_values:  # Verifica se há valores no item selecionado
                
                id_schedule = item_values[0]  # Seleciona o ID(Que está na posição [0])
                edit_screen = EditScheduleScreen(self, id_schedule)
                edit_screen.grab_set()  
                edit_screen.focus_set()  
                self.wait_window(edit_screen)  
                
            else:
                messagebox.showwarning("Warning", "Selected item has no values.")
